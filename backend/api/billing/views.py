...
class StoreQuoteAPIView(APIView):
    """Authenticated, server-authoritative quote for proposed Store selections."""

    def post(self, request):
        try:
            quote = build_store_quote(user=request.user, selection=request.data)
        except StoreQuoteValidationError as exc:
            return Response(
                {"code": exc.code, "detail": exc.detail},
                status=exc.status_code,
