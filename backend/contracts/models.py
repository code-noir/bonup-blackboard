...
class LifecycleAgreement(models.Model):
    ...
    performance_ready = models.BooleanField(default=False)
    performance_ready_at = models.DateTimeField(null=True, blank=True)
    performance_ready_by = models.ForeignKey(
        settings.AUTH_USER_MODEL,
        on_delete=models.SET_NULL,
        null=True,
        blank=True,
        related_name="performance_ready_lifecycle_agreements",
    )
    metadata = models.JSONField(default=dict, blank=True)
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
