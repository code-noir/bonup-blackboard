...
EVENTS = frozenset({'MODEL_PROPOSAL_RECEIVED', 'OPERATION_AUTHORIZED', 'OPERATION_DENIED',
                   ...
                   'FOUNDER_CHALLENGE_ISSUED', 'FOUNDER_SIGNATURE_ACCEPTED', 'FOUNDER_SIGNATURE_DENIED', 'FOUNDER_SESSION_CREATED', 'FOUNDER_SESSION_EXPIRED', 'INSTALLATION_APPROVAL_ISSUED', 'INSTALLATION_APPROVAL_DENIED', 'HOST_TEST_AUTHORIZATION_ACCEPTED', 'HOST_TEST_AUTHORIZATION_DENIED', 'HOST_TEST_SESSION_BEGUN', 'HOST_TEST_SESSION_ENDED', 'HOST_TEST_CASE_ENROLLED', 'HOST_TEST_CASE_STARTED', 'HOST_TEST_CASE_PASSED', 'HOST_TEST_CASE_FAILED', 'HOST_TEST_COMPLETION_ACCEPTED', 'HOST_TEST_COMPLETION_DENIED', 'HOST_TEST_REQUEST_RECEIVED','HOST_TEST_AUTHORIZED','HOST_TEST_REBOOT_PENDING','HOST_TEST_CLOSURE_REQUESTED','HOST_TEST_SUPERVISOR_CLOSED','HOST_TEST_CLOSURE_UNCERTAIN','HOST_TEST_AUTHORITY_CONSUMED'})
...
class Controller:
    ...
    def _check(self, enrollment, proposal):
        ...
        try:
            ...
            for paths, rules, write in ((binding.profile.readable_paths, readable, False),
                ...
                for path in paths:
                    if getattr(binding.root, 'filesystem_policy_digest', None) is not None:
                        binding.root.check_export_denials(path, (*protected, *task_denied,
                            *(task_readonly if write else ())))
    ...
    def _snapshot(binding, enrollment, command):
        ...
        return digest({
            ...
            'filesystem_policy': getattr(binding.root, 'filesystem_policy_digest', None),
            'resource_policy': IntegrationPolicy().policy_digest,
