...
class ExecutionDeadline:
    ...
    def arm(cls, expires_at, *, now, elapsed, timeout_seconds):
        if any(type(v) not in (int,float) or not math.isfinite(v) for v in (elapsed,timeout_seconds)):
            raise ValidationError('Finite authority lifetime required.')
...
class Supervisor:
    ...
    def prepare(self, launch_id):
        ...
        try:
            ...
            from .resource_supervision import authority_deadline
            deadline = authority_deadline(launch['deadline'],
                tuple(l['expires_at'] for l in parse_json(launch['binding'])['required_leases']),
                now=now, elapsed=elapsed, operation_seconds=record.timeout_seconds,
                operation_elapsed=record.elapsed_deadline)
    ...
    def release(self, launch_id):
        ...
        try:
            ...
            with self.runtime.transaction('release'):
                ...
                proof=self.backend.release(launch,record)
                if type(proof) is not ExecStart:
                    raise AuthorityError('Execution-start evidence required before RUNNING.')
