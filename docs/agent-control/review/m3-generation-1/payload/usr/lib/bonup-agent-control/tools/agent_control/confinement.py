...
class TaskRoot:
    ...
    @classmethod
    def adopt(cls, path, fd, expected_identity):
        """Take ownership of an already safely opened root; never reopen its path."""
        root = cls.__new__(cls)
        root.path, root.fd = path, fd
        root.identity = root._identity(os.fstat(fd))
        try:
            if root.identity != expected_identity:
                raise ValidationError('Pinned root identity mismatch.')
            root.verify()
            return root
