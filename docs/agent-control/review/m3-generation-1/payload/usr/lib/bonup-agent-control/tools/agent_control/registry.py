...
class Registry:
    ...
    def routing_event(self, event):
        """Controller-only observation, using the existing audit chain and outbox.

        This does not mint an AuthenticatedContext or grant agent/human authority.
        Audit failure propagates before further lifecycle work can proceed.
        """
        from dataclasses import asdict
        from .execution import RoutingEvent
        if type(event) is not RoutingEvent:
            raise ValidationError('Typed controller routing event required.')
        for value in (event.correlation_id, event.execution_id, event.launch_id):
            if value is not None and not valid_format('uuid', value):
                raise ValidationError('Invalid routing correlation.')
    ...
    def _operation(self, operation_id, kind, payload, context, action, event_type, *, bootstrap=False, routing=None):
        ...
        try:
            ...
            if routing is not None:
                event.update(actor={'component':'CONTROLLER'}, execution_id=routing.execution_id,
                    reason_code=routing.reason_code, routing=dict(request_id=routing.correlation_id,
                    launch_id=routing.launch_id))
                event['event_digest']=digest(event)
                validate_schema('AuditEvent',event)
            elif context:event=AuditEvent.finalize(event,context=context).to_dict()
            else:
                event['event_digest']=digest(event)
                validate_schema('AuditEvent',event)
