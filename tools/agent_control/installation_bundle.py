...
def projection_scope(*, poll_interval_ms=None, batch_size=None, timeout_ms=None,
    ...
    scope = dict(
        ...
        registry=dict(path=REGISTRY_PATH, history_path=HISTORY_PATH,
                      target_schema_version=REGISTRY_TARGET_VERSION,
                      initialization='INITIALIZE_ABSENT_THEN_MIGRATE_V1_TO_V2_TO_V3',
                      accepted_existing_versions=[1, 2, 3],
                      verify_history=True, preserve_existing_records=True,
                      reinitialize_existing=False),
...
def validate_projection_scope(scope):
    ...
    keys(scope, ('version', 'registry', 'event_config_path', 'event_config', 'event_config_file',
    ...
    keys(scope['registry'], ('path', 'history_path', 'target_schema_version',
                             'initialization', 'accepted_existing_versions',
                             'verify_history', 'preserve_existing_records',
                             'reinitialize_existing'))
    if scope['registry'] != {
            'path': REGISTRY_PATH, 'history_path': HISTORY_PATH,
            'target_schema_version': REGISTRY_TARGET_VERSION,
            'initialization': 'INITIALIZE_ABSENT_THEN_MIGRATE_V1_TO_V2_TO_V3',
            'accepted_existing_versions': [1, 2, 3],
            'verify_history': True, 'preserve_existing_records': True,
            'reinitialize_existing': False}:
        raise ValidationError('Invalid Registry installation scope.')
